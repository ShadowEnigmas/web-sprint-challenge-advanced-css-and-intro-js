...
function get20s(newArray){
  newArray = [];
  for(let i = 0; i < artists.length; i++){
    if(artists[i].years.charAt(0) == 1 && artists[i].years.charAt(1) == 9 && artists[i].years.charAt(7) == 1 && artists[i].years.charAt(8) == 9){
...
*/
function removeArtist(givenArray, index){
  newArray = [...givenArray];
  artistRemoved = newArray;
  newArray.splice(index, 1);
  console.log(artistRemoved.length);
}
...
function addArtist(name, years, genre, nationality){
  newArray = [...artists];
  ...
  addArtist("Robert", "1997 - 2020", "Full-Stack-Web", "American");
...
function lotsOfArt(givenArray, newArray){
  newArray = [];
  for(let i = 0; i < givenArray.length; i++){
    if(givenArray[i].paintings >= 100){
      newArray.push(givenArray[i].name);
    }
  }
  console.log(newArray);
}
